...
recent_data = [entry for entry in data if datetime.strptime(entry['date'], "%Y-%m-%d") >= seven_days_ago]

# 벡터 추출
vectors = np.array([entry['vector'] for entry in recent_data])

# 결과 파일 경로 설정
RESULT_FILE_DIR = './c3_7day_treemap'
...
for category, file_name in zip(categories, category_files):
    ...
    for i, label in enumerate(assigned_clusters):
        ...
        if similarity >= SIMILARITY_THRESHOLD:
            ...
            valid_clusters.append(recent_data[i])
    
    clustered_data = [{"cluster": name, "count": count} for name, count in cluster_counts.items() if count > 0]
    
    # 결과 파일 경로 설정 및 저장
    result_file_name = f'c3_treemap_{category}_{current_time_str}.ndjson'
    ...
    chart_file_name = f'c3_treemap_{category}_{current_time_str}.png'
